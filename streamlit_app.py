...
# NEW AWS ACCOUNT / NEW S3 BUCKET CONFIGURATION

# ============================================================

AWS_REGION = "eu-north-1"

DEFAULT_BUCKET = "terranova818-datasets-2026-454492134704-eu-north-1-an"
...
st.caption(
    "These columns show which model produced each prediction field in the combined-best dataset."
)
...
if summary_frames:
    ...
    pivot = (
        source_summary_df.pivot(
            index="prediction_field",
...
```
model_names = list(MODEL_S3_KEY_DEFAULTS.keys())

selected_model_name = st.sidebar.selectbox(
    "S3 model source",
    model_names,
    index=model_names.index("Combined Best"),
)
...
with st.sidebar.expander("Current S3 paths", expanded=False):
    st.write("Bucket:")
    st.code(bucket)
    st.write("Predictions key:")
    st.code(pred_key)
...
try:
    ...
    try:
        ...
        metrics_df = None
        st.sidebar.warning(
            f"{metrics_label} was not found or could not be read from S3."
        )
...
"Upload metrics/model-selection CSV",
type=["csv"],
)
...
if df_city.empty:
    ...
    if chosen_row is not None:
        ...
        st.write(
            f"Overall risk level: **{str(getattr(result, 'risk_level', 'unknown')).upper()}**"
        )
        ...
        if not cards:
            ...
            for c in cards:
                with st.container(border=True):
                    ...
                    st.write(
                        f"**Time to act:** {c['time_to_act']} | **Cost:** {c['cost_level']}"
                    )
